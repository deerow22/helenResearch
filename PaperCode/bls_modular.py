...
def open_mylcs(tic,sector,filepath):
    '''
...
######################## useful tools #########################
def period_grid(start=0.2,stop=27,N=25000,log='off'):
    '''
    ...
    if log=='on': #change test to gridlog 
        logstart = np.log10(start); logstop = np.log10(stop) #p_grid_log = np.logspace(start,stop,N) 
        p_grid = np.logspace(logstart, logstop, N)#np.log10(p_grid_log) #converts log2linear space - required for bls to work
    else:
        p_grid = np.linspace(start,stop,N)
    return p_grid

def duration_grid(start=.01,stop=0.19,N=5):
    '''
...
def plot_lc(time,flux,flux_err='None',ID = 'Light Curve - no id'):
    ...
    if flux_err != 'None':
        ...
        plt.errorbar(time,flux,yerr=flux_err,color='lightgray',lw=.5)
...
def finegrid(period):
	val=0.0625
	minp = (period)-(val); maxp=(val)+(period)
	finegrid = np.arange(minp,maxp,5e-6)
	return finegrid

def halfgrid(period):
	val = 0.0625
	minp = (period/2)-(val);maxp=(val)+(period/2)
	halfgrid = np.arange(minp,maxp,5e-6)
	return halfgrid
...
def dubgrid(period):
	val=0.0625
	minp = (period*2)-(val); maxp=(val)+(period*2)
	dubgrid = np.arange(minp,maxp,5e-6)
	return dubgrid
