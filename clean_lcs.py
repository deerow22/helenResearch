...
def sector_ordered_files(filepaths,sectors_list):
    '''
...
def clean_files(lightkurvefiles):
    ...
    if len(lcfs) ==1: #dont stitch or put in collection if single sector
        detrend = lcfs[0].PDCSAP_FLUX #this detrends/normalizes each sector
...
def save_lc(data,savepath=None):
    '''
    ~ Writes lightcurve data to file with specified path, creating directories if needed by path~
